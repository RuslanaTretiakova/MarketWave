...
- **Access**: Supabase clients in [`lib/supabase/`](../lib/supabase/) (see [AGENTS.md](ai/AGENTS.md) for which client to use)

Do not rely on this file for column-level detail when migrations disagree—**migrations always win**.

## Public tables (quick ref)

Same list as [AGENTS.md](ai/AGENTS.md) DB quick ref:
...
3. Disable **public sign-up** in Auth settings; use **in-app Settings → Team invitations** (admin only) for everyone else. Do not commit credentials in the repo.
...
- Regenerate TypeScript types after schema changes (command in [AGENTS.md](ai/AGENTS.md))
