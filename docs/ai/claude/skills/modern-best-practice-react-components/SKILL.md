...
- **Data:** Prefer fetching in **React Server Components** and mutating via **Server Actions** with Supabase ([AGENTS.md](../../../AGENTS.md)). **TanStack Query** is not a dependency here; add it only if a feature truly needs client caching, background refetch, or subscriptions.
- **Styling / routing:** Spacing tokens and `@theme` live in `app/globals.css`; edge auth uses `proxy.ts` (Next 16), not `middleware.ts`.
...
- **AVOID** `useEffect()`
  ...
  - Prefer **Server Components** for fetching; use **TanStack Query** (`@tanstack/react-query`) only when a feature needs client-side caching or subscriptions (opt-in dependency in this repo)
