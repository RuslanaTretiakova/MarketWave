...
One invoice per `(client_id, billing_month)` — monthly aggregated billing container for many orders.

| Column                     | Type             | Notes                                                              |
| -------------------------- | ---------------- | ------------------------------------------------------------------ |
| `id`                       | UUID             | PK                                                                 |
| `client_id`                | UUID             | NN FK→`profiles(id)` ON DELETE RESTRICT                            |
| `billing_month`            | DATE             | NN, first day of month (e.g. `2026-05-01`)                         |
| `status`                   | `invoice_status` | NN, D=`draft` (`draft` · `sent` · `paid`)                          |
| `subtotal`                 | NUMERIC(10,2)    | NN D=0, SUM of `invoice_items.amount`, synced by trigger           |
| `adjustments`              | NUMERIC(10,2)    | NN D=0, surcharges or discounts; CHECK `subtotal+adjustments >= 0` |
| `total`                    | NUMERIC(10,2)    | NN D=0, `subtotal + adjustments`, synced by trigger                |
| `amount`                   | NUMERIC(10,2)    | Legacy field kept in sync with `total` for back-compat             |
| `invoice_number`           | TEXT             | UQ, human-readable e.g. `INV-2026-0001`; auto-assigned on INSERT   |
| `due_date`                 | DATE             |                                                                    |
| `sent_at`                  | TIMESTAMPTZ      |                                                                    |
| `paid_at`                  | TIMESTAMPTZ      |                                                                    |
| `generated_at`             | TIMESTAMPTZ      | Set by `generate_monthly_invoices()` cron function                 |
| `sent_by`                  | UUID             | FK→`profiles(id)` ON DELETE SET NULL                               |
| `paid_by`                  | UUID             | FK→`profiles(id)` ON DELETE SET NULL                               |
| `notes`                    | TEXT             |                                                                    |
| `order_id`                 | UUID             | Nullable FK→`orders(id)` — legacy, no longer written               |
| `invoice_group_id`         | UUID             | Legacy grouping field — no longer written                          |
| `created_at`, `updated_at` | TIMESTAMPTZ      |                                                                    |

- **Indexes:** `idx_invoices_status`, `idx_invoices_billing_month`, `idx_invoices_client_id`, `idx_invoices_client_billing_month`, `idx_invoices_status_billing_month`, `idx_invoices_invoice_number`.
- **Partial unique:** `uq_invoices_client_month_draft ON (client_id, billing_month) WHERE status = 'draft'` — at most one draft invoice per client per month.
- **Triggers:**
  - `set_updated_at`
  - `trg_assign_invoice_number` (BEFORE INSERT) — assigns `invoice_number` from sequence if NULL.
  - `on_invoice_items_changed` → `sync_invoice_totals_from_items()` (AFTER INS/UPD/DEL on `invoice_items`) — recomputes `subtotal`, `total`, `amount`.
  - `on_invoice_adjustment_changed` → `sync_invoice_total_on_adjustment()` (BEFORE UPDATE OF `adjustments`) — recomputes `total`, `amount`.
  - `on_invoice_status_changed` → `handle_invoice_paid()` (BEFORE UPDATE OF `status`) — when status → `paid`, sets `paid_at` and flips all attached `published` orders → `completed`.
  - `enforce_invoice_row_mutability` → `enforce_invoice_mutability()` (BEFORE UPDATE/DELETE) — blocks field edits on `sent` invoices (only `status → paid` allowed); blocks all edits on `paid` invoices.
- **RLS:**
  - SELECT: client sees own `sent`/`paid` invoices (`client_id = auth.uid() AND status IN ('sent','paid')`); admin/manager see all.
  - UPDATE: admin/manager only (further restricted by mutability trigger).
  - INSERT/DELETE: service role only (cron function + Server Actions via `adminClient`).

### `invoice_items`

Each row references one published/completed order. An order can appear in at most one invoice across the entire table (`UNIQUE order_id`).
...
| `site_domain`              | TEXT          | Snapshot of site domain at time of attachment |
| `description`              | TEXT          | Optional free-text override                   |
| `amount`                   | NUMERIC(10,2) | NN CHECK ≥0                                   |
| `created_at`, `updated_at` | TIMESTAMPTZ   |                                               |

- **Indexes:** `idx_invoice_items_invoice_id`, `uq_invoice_items_order_id` (UNIQUE).
- **Triggers:**
  - `set_updated_at_invoice_items`
  - `on_invoice_items_changed` → `sync_invoice_totals_from_items()` — keeps parent invoice totals in sync.
  - `enforce_invoice_items_mutability` → `enforce_invoice_mutability()` (BEFORE INS/UPD/DEL) — blocks changes when parent invoice is `sent` or `paid`; on INSERT validates `orders.user_id = invoices.client_id` and `orders.status IN ('published','completed')`.
- **RLS:**
  - SELECT: client sees items for own `sent`/`paid` invoices; admin/manager see all.
  - INSERT/UPDATE/DELETE: admin/manager only (trigger further restricts to `draft` invoices).
...
- ~~`handle_new_order()`~~ — **removed** in `20260705000001`. Invoices are no longer auto-created per order; generated monthly by cron instead.
- ~~`sync_draft_invoice_item_from_order_price()`~~ — **removed** in `20260705000001`.
- `sync_invoice_totals_from_items()` — AFTER INS/UPD/DEL on `invoice_items`; recomputes `invoices.subtotal`, `total`, `amount`.
- `sync_invoice_total_on_adjustment()` — BEFORE UPDATE OF `adjustments` on `invoices`; recomputes `total`, `amount`.
- `enforce_invoice_mutability()` — BEFORE INS/UPD/DEL on `invoice_items` and BEFORE UPD/DEL on `invoices`; blocks edits on `sent`/`paid` invoices; validates order eligibility on item INSERT.
- `handle_invoice_paid()` — BEFORE UPDATE OF `status` on `invoices`; when status → `paid`, sets `paid_at` and flips **all** attached `published` orders → `completed` (iterates `invoice_items`).
- `generate_monthly_invoices(p_billing_month DATE) RETURNS INT` — SECURITY DEFINER function; idempotent; for every client with uninvoiced `published` orders in the given month, upserts a draft invoice and inserts the corresponding `invoice_items`. Called by cron and by the admin UI.
- `assign_invoice_number()` — BEFORE INSERT on `invoices`; assigns `INV-YYYY-NNNN` from sequence if NULL.
...
| `invoices`                               | client: own `sent`/`paid` via `client_id`; admin/manager: all | service role only                                           | admin/manager (trigger blocks sent/paid)               | service role                             |
| `invoice_items`                          | client: own `sent`/`paid` parent; admin/manager: all          | admin/manager (trigger validates order eligibility)         | admin/manager (trigger blocks sent/paid)               | admin/manager (trigger blocks sent/paid) |
...
- **Cron (`pg_cron`):**
  - `cleanup-read-notifications` — Sun 03:00 UTC, deletes read notifications >7 days old.
  - `generate-monthly-invoices` — 1st of every month 02:00 UTC, calls `generate_monthly_invoices(date_trunc('month', now() - interval '1 month')::date)` to create draft invoices for the previous month.
...
- **`lib/notifications/notify-order-event.ts`** — two dispatchers:
  - `notifyOrderEvent(event, ctx)` — per-order events (assigned, submitted, published, etc.). Resolves recipients per event, generates **role-specific second-person copy**, batches into one `notifications` insert via `adminClient`. Fire-and-forget.
  - `notifyInvoiceEvent(event, ctx)` — invoice-level events (`invoice_sent`, `invoice_paid`). Loads the invoice for billing month / total / item count; generates copy referencing the invoice number and month (not a single order); notifies client + all managers. Fire-and-forget.
...
| `invoice_paid`                                                         | `markInvoicePaid` via `notifyInvoiceEvent`        | client (invoice owner), all managers      |
| `invoice_sent`                                                         | `sendInvoice` via `notifyInvoiceEvent`            | client (invoice owner), all managers      |
...
- Orders are **not** individually invoiced on INSERT. Instead, `generate_monthly_invoices()` runs on the 1st of each month and batches all uninvoiced `published` orders for the previous month into one draft invoice per client.
- Flipping an invoice → `paid` flips **all** attached `published` orders → `completed` via `handle_invoice_paid()` trigger (iterates `invoice_items`).
...
- **Invoice actions:** `lib/invoices/invoice-actions.ts` (`sendInvoice`, `markInvoicePaid`, `editInvoiceOrders`, `generateMonthlyInvoices`), `lib/invoices/load-invoices.ts` (`loadInvoicesPage`, `loadInvoiceDetail`).
- **Invoice UI:** `components/invoices/invoice-detail-view.tsx`, `components/invoices/edit-invoice-orders.tsx`, `components/invoices/invoices-list.tsx`, `components/invoices/statement-card.tsx`, `components/invoices/invoice-pdf-document.tsx`, `components/invoices/generate-monthly-invoices.tsx`.
- **Invoice PDF route:** `app/api/invoices/[invoiceId]/pdf/route.tsx`.
- **Invoice cron route:** `app/api/cron/create-invoices/route.ts` (HTTP trigger; cron also runs via `pg_cron` directly in DB).
